...
$viewname_it7 = 'Vue_actu_globale_items';
$view = views_get_view ($viewname_it7);
$viewdisplay_it7 = $view->set_display('block_7');
...
if (!empty($view->result)) {
  ...
  $output = '<div id="items_annonce"><h3 class="titre-pole-formation">'.$view->get_title().'</h3>' .$view->preview($viewdisplay_it7).'</div>';
...
elseif (empty($view->result)) {
    //Formatage du texte vide,ajout du titre de la vue
     $outputEmpty = '<div id="items_annonce"><h3 class="titre-pole-formation">'.$view->get_title().'</h3>' .$emptyText.'</div>';
