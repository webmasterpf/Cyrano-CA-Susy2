...
$viewname_it2 = 'Vue_actu_globale_items';
$view = views_get_view ($viewname_it2);
$viewdisplay_it2 = $view->set_display('block_2');
...
if (!empty($view->result)) {
  ...
  $output = '<div id="items_projet"><h3 class="titre-pole-formation">'.$view->get_title().'</h3>' .$view->preview($viewdisplay_it2).'</div>';
...
elseif (empty($view->result)) {
    //Formatage du texte vide,ajout du titre de la vue
     $outputEmpty = '<div id="items_sortie"><h3 class="titre-pole-formation">'.$view->get_title().'</h3>' .$emptyText.'</div>';
