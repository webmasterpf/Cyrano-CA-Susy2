...
$viewname_it3 = 'Vue_actu_globale_items';
$view = views_get_view ($viewname_it3);
$viewdisplay_it3 = $view->set_display('block_3');
...
if (!empty($view->result)) {
  ...
  $output = '<div id="items_innovation"><h3 class="titre-pole-formation">'.$view->get_title().'</h3>' .$view->preview($viewdisplay_it3).'</div>';
...
elseif (empty($view->result)) {
    //Formatage du texte vide,ajout du titre de la vue
     $outputEmpty = '<div id="items_innovation"><h3 class="titre-pole-formation">'.$view->get_title().'</h3>' .$emptyText.'</div>';
