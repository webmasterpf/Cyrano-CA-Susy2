...
$viewdisplay_ag7 =$view->set_display('block_7');
...
if (!empty($view->result)) {
  ...
  $output = '<div id="actu_globale_annonce"><h3 class="titre-pole-formation">'.$view->get_title().'</h3>' .$view->preview($viewdisplay_ag7).'</div>';
...
elseif (empty($view->result)) {
    //Formatage du texte vide,ajout du titre de la vue
     $outputEmpty = '<div id="actu_globale_annonce"><h3 class="titre-pole-formation">'.$view->get_title().'</h3>' .$emptyText.'</div>';
