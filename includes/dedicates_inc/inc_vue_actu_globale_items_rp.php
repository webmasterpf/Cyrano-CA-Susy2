...
$viewname_it6 = 'Vue_actu_globale_items';
$view = views_get_view ($viewname_it6);
$viewdisplay_it6 = $view->set_display('block_6');
...
if (!empty($view->result)) {
  ...
  $output = '<div id="items_rp"><h3 class="titre-pole-formation">'.$view->get_title().'</h3>' .$view->preview($viewdisplay_it6).'</div>';
...
elseif (empty($view->result)) {
    //Formatage du texte vide,ajout du titre de la vue
     $outputEmpty = '<div id="items_rp"><h3 class="titre-pole-formation">'.$view->get_title().'</h3>' .$emptyText.'</div>';
